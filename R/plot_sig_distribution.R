...
plot_sig_percentage <- function(sample_sig_count, SampleCol, h,w,text_size,outputname){
  ...
  p <- ggplot2::ggplot(data=sample_sig_per_melt, aes(x=Sample, y=Count,fill=Sig))+ ggplot2::geom_bar(stat="identity", position = "fill")+ggplot2::xlab("Sample")+ggplot2::ylab("Percentage")
...
plot_sig_count <- function(sample_sig_count, SampleCol, h,w,text_size,outputname){
  ...
  names(sample_sig_per_melt) <- c("Sample", "Sig", "Count")
  grDevices::pdf(file=paste0(outputname,".pdf"), onefile=TRUE,width=w,height=h)
  p <- ggplot2::ggplot(data=sample_sig_per_melt, aes(x=Sample, y=Count,fill=Sig))+ ggplot2::geom_bar(stat="identity")+ggplot2::xlab("Sample")+ggplot2::ylab("Count")
